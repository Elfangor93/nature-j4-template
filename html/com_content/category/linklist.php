...
$hhtag = $this->params->get('show_page_heading') ? 'h3' : 'h2';
?>

<div class="com-content-category-list link-list">
  ...
  <?php echo HTMLHelper::_('content.prepare', '{loadposition content-between}'); ?>
  ...
  <form action="<?php echo htmlspecialchars(Uri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm" class="com-content-category__articles">
    ...
    <?php else : ?>
      ...
      <table class="com-content-category__table category table table-striped table-bordered table-hover">
        ...
        <tbody>
        <?php foreach ($this->items as $c => $cats) : ?>
          <tr class="cat-list-heading">
            <th class="list-title">
              <?php echo $c; ?>
            </th>
          </tr>

          <?php foreach ($cats as $i => $article) : ?>
            <?php if ($this->items[$c][$i]->state == ContentComponent::CONDITION_UNPUBLISHED) : ?>
            ...
            <?php endif; ?>
              ...
              <?php if (in_array($article->access, $this->user->getAuthorisedViewLevels())) : ?>
                ...
                <td class="list-links">
                  <div class="btn-group">
                    <?php foreach ($urlarray as $key => $url) :
                      $link = $url[0];
                      $label = $url[1];
                      $target = $url[2];
                      $id = $url[3];
